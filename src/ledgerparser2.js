const fromLedger = function(ledger) {
    ...
    for (var i = 0; i < lines.length; i++)
    {
        ...
        else if (lines[i].match(/^(  |\t)/) && blocks.length > 0 && (blocks[blocks.length-1].type == "transaction" || blocks[blocks.length-1].type == "other"))
        {
            ...
            else
            {
                ...
                if (line.length > 2)
                {
                    let candidateStatus = line.substring(0, 2);
                ...
                // Look for amounts
                let postingSplitRegex = /\s*(  |\t)\s*/g;
                let group = postingSplitRegex.exec(line);
                let account = null;
                let remainingPart = "";
                let amount = "";
                ...
                {
                    let remaining = commentRegex.exec(remainingPart);
                ...
                blocks[blocks.length-1].lines.push({type: "posting", account: account, amount: amount, comment: comment, status: status});
    ...
    return {blocks: blocks, transactionIndexes: transactionIndexes, accounts: Object.keys(accountList)};
};
    ...
    for (var i = 0; i < objs.blocks.length; i++)
    {
        ...
        else if (objs.blocks[i].type == "transaction" || objs.blocks[i].type == "other")
        {
            ...
            if (objs.blocks[i].lines)
            {
                ...
                let longestAmount = 0;
                for (var j = 0; j < objs.blocks[i].lines.length; j++)
                {
                    if (objs.blocks[i].lines[j].type == "posting")
                    {
                        let candidateLongestAccount = objs.blocks[i].lines[j].account.length + (objs.blocks[i].lines[j].status.length > 0 ? 2 : 0);
                ...
                for (var j = 0; j < objs.blocks[i].lines.length; j++)
                {
                    if (objs.blocks[i].lines[j].type == "posting")
                    {
                        ...
                        let statusLength = objs.blocks[i].lines[j].status.length > 0 ? 2 : 0;
